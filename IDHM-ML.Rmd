...
```
Remoção das variáveis dependentes da variável alvo `ESPVIDA` (expectativa de vida ao nascer), i.e., que vazam dados:

$$IDHM\_L = (ESPVIDA - 25) / (85 - 25)$$
$$IDHM = (IDHM\_E * IDHM\_L * IDHM\_R) ^{(1/3)}$$
```{r}
data <- select(data, -c("IDHM", "IDHM_L"))
```
Variáveis altamente correlacionadas, i.e., limite absoluto [0.8]
```{r}
...
control <- trainControl(method="cv", number=2, verboseIter=TRUE)
ptm <- proc.time()
fit <- train(ESPVIDA ~ ., data=data[-highlyCorrelated], method="rf", preProcess="scale", trControl=control, importance=TRUE)
proc.time() - ptm
fit
``` 
```{r}
