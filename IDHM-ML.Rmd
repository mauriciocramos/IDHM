...
### 4 - Default Random Forest e reamostragem por validação cruzada 3 k-fold
```{r}
set.seed(1)
ptm <- proc.time()
fit <- train(ESPVIDA ~ ., training, method="rf", preProcess=NULL, trControl=trainControl(method="cv", number=3, search="grid", verboseIter = TRUE), tuneGrid=NULL, tuneLength=3)
proc.time() - ptm
fit
postResample(predict(fit, testing), testing$ESPVIDA)
```

### 5 - Default Random Forest e reamostragem por validação cruzada 5 k-fold
```{r}
set.seed(1)
ptm <- proc.time()
...
### 6 - Default Random Forest e reamostragem por validação cruzada 10 k-fold
```{r}
set.seed(1)
ptm <- proc.time()
fit <- train(ESPVIDA ~ ., training, method="rf", preProcess=NULL, trControl=trainControl(method="cv", number=5, search="grid", verboseIter = TRUE), tuneGrid=NULL, tuneLength=3)
