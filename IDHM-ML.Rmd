...
system("ps -f | grep -e mauric+ -e parallel")
```
...
```
Remoção das variáveis dependentes da variável alvo `ESPVIDA` (expectativa de vida ao nascer) pois seriam um vazamento de dados.
...
```
Variáveis altamente correlacionadas, i.e., acima do limite [0.8]
```{r}
...
## Importância variáveis estimada pelo Random Forest

### Usando variáveis altamente correlacionadas:
```{r}
set.seed(1)
control <- trainControl(method="cv", number=10, verboseIter=TRUE) #?
# control <- trainControl(method="cv", number=2, verboseIter=TRUE) #1130s
ptm <- proc.time()
...
set.seed(1)
control <- trainControl(method="cv", number=10, verboseIter=TRUE) #?
# control <- trainControl(method="cv", number=2, verboseIter=TRUE) #335s
ptm <- proc.time()
