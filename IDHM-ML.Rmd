...
models <- expand.grid(
    variables = list(noHighCor=names(data)[! names(data) %in% highlyCorrelated],
                     all=names(data)),
    percentage = list(.1, .2, .3, .4, .5, .6, .7, .8, .9),
    ...
    method = list("glm", "svmLinear2", "knn", "rf", "rpart", "gbm", "knn", "xgbTree"))
models <- as.list(as.data.frame(t(models), optional = TRUE))
```
```{r}
for(i in seq(models)) {
    with(models[[i]], message(i,"/",length(models)," Training: vars=", length(variables), 
                              " percent=", percentage, " pre=(", paste(preProcess, collapse=","),
                              ") method=", method))
    set.seed(1)
    inTraining <- createDataPartition(data$ESPVIDA, p = models[[i]]$percentage, list = FALSE)
    time <- proc.time()
    models[[i]]$fit <- train(ESPVIDA ~ ., data[inTraining, models[[i]]$variables],
                             #importance=NULL, #rf
                             #verbose=FALSE, #gbm
    ...
    models[[i]]$performance <- postResample(predict(models[[i]]$fit,
...
```
```{r}
bind_rows(lapply(models, function(x) data.frame(variables=length(x$variables),
