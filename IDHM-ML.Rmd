...
Pacotes R:
```{r}
packages <- c("plyr", "ggplot2", "gridExtra", "GGally", "dplyr", "tidyr", "caret", "parallel", "doParallel", "randomForest") #e1071
...
models <- expand.grid(
    ...
    partition = list(.1),#, .3, .4, .5, .6, .7, .8, .9),
...
```{r}
for(i in seq(models)) {
    with(models[[i]], message("\n[",Sys.time(),"] #",i,"/",length(models)," training: nvar=", length(variables), 
                              " %rows=", partition, " preProc=(", paste(preProcess, collapse=","),
                              ") method=", method))
    set.seed(1)
    inTraining <- createDataPartition(data$ESPVIDA, p = models[[i]]$partition, list = FALSE)
    time <- proc.time()
    models[[i]]$train <- train(ESPVIDA ~ ., data[inTraining, models[[i]]$variables],
                               #importance=NULL, #rf
                               #verbose=FALSE, #gbm
    ...
    models[[i]]$performance <- postResample(predict(models[[i]]$train,
                                                    data[-inTraining, models[[i]]$variables]),
                                            data$ESPVIDA[-inTraining])
    rm(time); rm(inTraining)
