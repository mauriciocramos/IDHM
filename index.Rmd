---
title: "Machine Learning com o Indice de Desenvolvimento Humano Municipal do Brasil"
...
Linguagem `r R.version.string`

Pacotes R:
```{r}
options(java.parameters="-Xmx8192m")
packages <- c("ggplot2", "gridExtra", "GGally", "dplyr", "tidyr", "caret")
...
grid.arrange(layout_matrix=rbind(c(1,2,2,2), c(3)), heights=c(1,2), top = "População",
             ...
             ggplot(estadual, aes(REGIAO, PESOTOT/1e6, fill=REGIAO)) +
                 ...
                 theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_grid(cols=vars(ANO)),
             ggplot(estadual, aes(UFN, PESOTOT/1e6, fill=REGIAO)) + 
                 ...
                 theme(axis.text.x = element_text(angle = 45, hjust = 1)) + facet_grid(rows=vars(ANO)))
```
...
ggplot(estadual, aes(FECTOT, ESPVIDA)) +
    ...
    facet_grid(cols=vars(ANO)) + geom_smooth(method="loess", color="black", size=.5, se = FALSE)
```
...
```{r}
municipal2010 <- filter(municipal, ANO=="2010")[1:1000,]
