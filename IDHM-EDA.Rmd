...
Em cada grupo, se duas variáveis possuírem correlação alta, no caso, maior que [`r cutoff`], é sugerido ao modelo preditivo a variável que possuir menor correlação média absoluta dentro do grupo.  As demais variáveis com baixa correlação dentro do grupo também são sugeridas ao modelo preditivo.

```{r}
proposeVars <- function(df, target, vars, cutoff) {
  correlationMatrix <- cor(select(df, vars, -target))
  highlyCorrelated <- findCorrelation(correlationMatrix, cutoff=cutoff, names=TRUE)
  vars[vars != target & ! vars %in% highlyCorrelated]
}
...
Variáveis sugeridas:
```{r}
(proposedVars <- proposeVars(municipal2010, target, vars, cutoff))
```
Estas variáveis serão avaliadas na modelagem preditiva.
```{r}
innerGroupVars <- c(innerGroupVars, proposedVars)
```
...
```{r}
proposedVars <- proposedVars[proposedVars!="IDHM_L"]
innerGroupVars <- c(innerGroupVars, proposedVars)
```
...
```{r}
cat(length(innerGroupVars), "preditores:\n")
cat(paste(innerGroupVars <- unique(innerGroupVars), collapse=", "))
```

Proposta de preditores que não estejam altamente correlacionados entre seus grupos.
```{r}
betweenGroupVars <- proposeVars(municipal2010, target, innerGroupVars, cutoff)
cat(length(betweenGroupVars), "preditores:\n")
cat(paste(betweenGroupVars, collapse=", "))
```

Proposta de preditores que não estejam altamente correlacionados em geral.
```{r}
allVars <- proposeVars(municipal2010, target, names(municipal2010[,-(1:8)]), cutoff)
cat(length(allVars), "preditores:\n")
